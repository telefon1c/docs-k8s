...
- Retype v4 (Pro) генерирует статику, nginx раздаёт HTML с CDN rewrite для ассетов
...
- [x] Фаза 1 — CI/CD: reusable workflow (`retypeapp/action-build@latest`, dotnet 10.x, `key`), Docker build → ghcr.io, S3 upload ассетов
